...
const Pay = require('../backend/models/Pay');
...
const mangaRoutes = require('../backend/routes/mangaRoutes');
const serieRoutes = require('../backend/routes/serieRoutes');
const payRoutes = require('../backend/routes/payRoutes');
const adminRoutes = require('../backend/routes/adminRoutes');
const {
  createVerifiedUser, formatCreateUserError, setJwtCookie, setVipCookie
} = require('../backend/controllers/homeController');
const createToken = require('../backend/helpers/createToken');
const vipToken = require('../backend/helpers/vipToken');
...
app.post('/send-email', async (req, res) => {
  try {
    ...
    await storePendingRegistration(trimmedEmail, { otp, name: trimmedName, password: String(password) });
...
app.post('/verify-otp/again', async (req, res) => {
  try {
    ...
    const trimmedEmail = String(email || '').trim().toLowerCase();
    if (!trimmedEmail) return res.status(400).json({ ok: false, error: 'Email is required' });
    if (!otp) return res.status(400).json({ ok: false, error: 'OTP is required' });
    if (!password || String(password).length < 6) {
      return res.status(400).json({ ok: false, error: 'Password must be at least 6 characters' });
    }
    const pending = await getPendingPasswordReset(trimmedEmail);
    if (!pending || Date.now() > pending.expiresAt) {
      if (pending) await Otp.deleteOne({ _id: pending._id });
...
app.post('/vip/access/:id', AdminMiddleware, async (req, res) => {
  try {
    ...
    if (!payment) return res.status(404).json({ ok: false, error: 'Payment request not found' });
    ...
    return res.json({
      ...
      user: { id: user._id, name: user.name, email: user.email, vipInvitePending: user.vipInvitePending, vipDuration: user.vipDuration },
    });
  } catch (e) {
...
app.get('/vip/pending', AuthMiddleware, async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ ok: false, error: 'User not found' });
    return res.json({ ok: true, pending: Boolean(user.vipInvitePending), vipMember: Boolean(user.vipMember), vipDuration: user.vipDuration });
  } catch (e) {
...
app.post('/vip/del/:id', AdminMiddleware, async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      user: { id: user._id, name: user.name || payment.username, vipInvitePending: user.vipInvitePending, vipMember: user.vipMember },
    });
  } catch (e) {
...
app.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (String(otp) !== pending.otp) return res.status(401).json({ ok: false, error: 'Wrong OTP' });
    const newUser = await createVerifiedUser({ name: pending.name, email: trimmedEmail, password: pending.password });
    const token = createToken(newUser._id);
    setJwtCookie(res, token);
    await Otp.deleteOne({ _id: pending._id });
    return res.status(201).json({ ok: true, token, user: { id: newUser._id, name: newUser.name, email: newUser.email } });
  } catch (err) {
